...
const cleanReason = value => typeof value === 'string' && value.trim().length >= 3 && value.trim().length <= 500 ? value.trim() : null;
  ...
  const create = async (req, res) => {
    ...
    try {
      ...
      const resolved = items.rows.map(item => {
        ...
        const priceOverrideReason = actual !== Number(item.quote_minor) ? cleanReason(input.priceOverrideReason) : null;
        const discountReason = discount > 0 ? cleanReason(input.discountReason) : null;
        if (actual !== Number(item.quote_minor) && !priceOverrideReason) throw new CheckoutError('CHECKOUT_PRICE_OVERRIDE_REASON_REQUIRED');
        if (discount > 0 && !discountReason) throw new CheckoutError('CHECKOUT_DISCOUNT_REASON_REQUIRED');
        return { ...item, actual, discount, final: actual - discount, priceOverrideReason, discountReason };
      });
      ...
      for (const item of resolved) await client.query(
        `INSERT INTO checkout_line_items(shop_id,checkout_id,appointment_item_id,line_type,description_snapshot,quote_price_minor,actual_price_minor,discount_minor,final_value_minor,price_override_reason,discount_reason)
         VALUES($1,$2,$3,'service',$4,$5,$6,$7,$8,$9,$10)`,
        [req.ownerAuth.shopId, checkout.rows[0].id, item.id, item.service_name_snapshot, item.quote_minor, item.actual, item.discount, item.final, item.priceOverrideReason, item.discountReason]
      );
      ...
      await client.query(
        `INSERT INTO checkout_financial_audit(shop_id,checkout_id,appointment_id,event_type,before_snapshot,after_snapshot,reason,operator_type,operator_id,source)
         VALUES($1,$2,$3,'checkout_created',NULL,$4,$5,'owner',$6,'owner_checkout_api')`,
        [req.ownerAuth.shopId, checkout.rows[0].id, appointmentId,
          JSON.stringify({ quoteTotalMinor: items.rows.reduce((n, i) => n + Number(i.quote_minor), 0), actualTotalMinor: resolved.reduce((n, i) => n + i.actual, 0), discountTotalMinor: resolved.reduce((n, i) => n + i.discount, 0), finalDueMinor: finalDue, paidMinor: paid, items: resolved.map(i => ({ appointmentItemId:i.id, quoteMinor:Number(i.quote_minor), actualMinor:i.actual, discountMinor:i.discount })) }),
          resolved.map(i => i.priceOverrideReason || i.discountReason).filter(Boolean).join(' | ') || null, req.ownerAuth.ownerAccountId]
      );
