...
const EXCEPTIONAL_COUNTRY_ISOS = Object.freeze(new Set(['AC', 'TA']));
const USER_ASSIGNED_COUNTRY_ISOS = Object.freeze(new Set(['XK']));

const NON_GEOGRAPHIC_CALLING_CODES = Object.freeze(new Set([
  '+800',
  '+808',
  '+870',
  '+878',
...
const getRegionKind = (countryIso2, callingCode) => {
  if (callingCode !== undefined && callingCode !== null) {
    ...
    if (NON_GEOGRAPHIC_CALLING_CODES.has(normalizedCode)) {
  ...
  if (EXCEPTIONAL_COUNTRY_ISOS.has(iso)) {
    return REGION_KINDS.EXCEPTIONAL;
  }
  if (USER_ASSIGNED_COUNTRY_ISOS.has(iso)) {
