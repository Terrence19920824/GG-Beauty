...
const resolveShop = async (client, shopSlug) => {
  const result = await client.query(
    `SELECT shop.id AS shop_id,settings.default_phone_country_code,settings.dob_requirement
...
const createCustomerMemberIdentity = ({ pool, getProvider, pepper, now = () => new Date() }) => {
  ...
  return { authenticate,confirmPhoneChange,normalizeSelectedPhone,requestOtp,verifySignIn };
};
