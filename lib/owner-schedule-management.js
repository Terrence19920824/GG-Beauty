...
const createOwnerScheduleManagement = ({ pool, isUuid, runInTransaction, safeErrorCode, invalidateCatalog }) => {
  ...
  const assertNoFutureConflict = async (client, { shopId, staffId, locationId, timezone, mode, days, override }) => {
    ...
    const result = await client.query(`
      ...
      WHERE item.shop_id = $1
        ...
        AND item.end_at > NOW()
        AND appointment.status IN ('pending', 'confirmed', 'arrived', 'in_service')
        AND ${predicate}
      LIMIT 1
    `, params);
  ...
  const putWeekly = async (req, res) => {
    ...
    try {
      ...
      if (typeof invalidateCatalog === 'function') invalidateCatalog(req.ownerAuth.shopId);
  ...
  const postOverride = async (req, res) => {
    ...
    try {
      const row = await runInTransaction(pool, async client => {
        ...
        `, [req.ownerAuth.shopId, value.locationId, req.params.staffId, value.scheduleDate, value.overrideType, value.startTime, value.endTime, value.reason ?? null, req.ownerAuth.ownerAccountId]);
