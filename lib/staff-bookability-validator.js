...
const VALIDATION_SQL = `
  WITH input AS (
    SELECT
      ...
      $7::UUID AS excluded_appointment_id
  ),
  identity_scope AS (
    SELECT
      input.*,
  ...
  collision_state AS (
    SELECT EXISTS (
      ...
      CROSS JOIN local_context AS context
      WHERE assignment.shop_id = context.shop_id
        AND assignment.staff_id = context.staff_id
        AND assignment.role IN ('primary', 'assistant')
        AND assignment.blocks_time = TRUE
        AND (
          context.excluded_appointment_id IS NULL
          OR collision_item.appointment_id <>
            context.excluded_appointment_id
        )
        AND assignment.start_at < context.requested_end_at
        AND assignment.end_at > context.requested_start_at
    ) AS has_collision
  )
  SELECT
...
const assertInput = input => {
  ...
  if (
    ...
    (
      ...
      !UUID_PATTERN.test(input.excludeAppointmentId)
    )
  ) {
    fail('BOOKABILITY_INPUT_INVALID');
  }
