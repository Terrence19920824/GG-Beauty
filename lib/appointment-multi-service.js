...
const createSingleServiceCompatibilityRows = async (
  client,
  { appointment, service, serviceLocale = null }
) => {
...
const createMultiServiceRows = async (client, { appointment, items, serviceLocale }) => {
  const created = [];
  for (const item of items) {
    const itemResult = await client.query(
      `INSERT INTO appointment_items (
       ...
       ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'booking',$10,$11,$12)
       RETURNING id`,
      [appointment.shop_id, appointment.location_id, appointment.id, item.serviceId,
       item.sequenceNo, item.localizedName, serviceLocale, item.durationMinutes,
       item.price, item.startAt, item.endAt, appointment.status]
    );
    if (itemResult.rows.length !== 1) throw new AppointmentMutationError('appointment_item_insert_mismatch', 500, '预约创建失败');
    const assignmentResult = await client.query(
      `INSERT INTO appointment_item_staff_assignments (
         shop_id,location_id,appointment_item_id,staff_id,role,start_at,end_at
       ) VALUES ($1,$2,$3,$4,'primary',$5,$6) RETURNING id`,
      [appointment.shop_id, appointment.location_id, itemResult.rows[0].id,
       item.staffId, item.startAt, item.endAt]
    );
    if (assignmentResult.rows.length !== 1) throw new AppointmentMutationError('appointment_assignment_insert_mismatch', 500, '预约创建失败');
    created.push({ itemId: itemResult.rows[0].id, assignmentId: assignmentResult.rows[0].id });
  }
  return created;
};
...
const loadAndValidatePhaseAStructure = async (
  ...
  const validationResult = await client.query(
    ...
    assignment_per_item AS (
      SELECT
        ...
        COALESCE(
          BOOL_AND(
            ...
            AND staff_member.id IS NOT NULL
      ...
      LEFT JOIN staff staff_member
        ON staff_member.shop_id = assignment.shop_id
       AND staff_member.id = assignment.staff_id
      GROUP BY item.id
    ),
    ...
    , primary_summary AS (
      SELECT
        COUNT(DISTINCT assignment.staff_id)::INTEGER AS primary_staff_count,
        MIN(assignment.staff_id::TEXT)::UUID AS sole_primary_staff_id
      FROM ordered_items item
    ...
    SELECT
      ...
      primary_summary.primary_staff_count,
      primary_summary.sole_primary_staff_id,
      (
        item_validation.item_count > 0
        AND item_validation.items_valid
        AND assignment_validation.assignments_valid
        AND (
          item_validation.item_count <> 1
          OR primary_summary.sole_primary_staff_id =
            (SELECT staff_id FROM parent)
        )
      ) AS structure_valid
    FROM item_validation
    CROSS JOIN assignment_validation
    CROSS JOIN primary_summary
    `,
    [
      appointment.shop_id,
      appointment.location_id,
  ...
  return {
    ...
    primaryStaffCount: validation.primary_staff_count,
    solePrimaryStaffId: validation.sole_primary_staff_id
  };
};
