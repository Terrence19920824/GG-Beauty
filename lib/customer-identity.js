...
const resolveVerifiedBooker = async (client,{shopId,session}) => {
  ...
  if (result.rows.length!==1) throw new CustomerIdentityError('VERIFIED_BOOKER_REQUIRED');
...
const resolveBookingParties = async (client, { shopId, body,verifiedSession=null }) => {
  ...
  const booker = await resolveVerifiedBooker(client,{shopId,session:verifiedSession}) ||
    await resolveOrCreateCustomer(client, { shopId, ...bookerDraft });
