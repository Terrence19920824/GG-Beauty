...
const createOwnerStaffCapabilityManagement = ({
    ...
    try {
      ...
      const result = await client.query(
        ...
        SELECT
          ...
          service.name,
          service.name AS canonical_name,
          service_translation_zh.name AS name_zh,
          service_translation_en.name AS name_en,
          service.category_id,
          service.category,
          category.canonical_name AS category_name,
          category_translation_zh.name AS category_name_zh,
          category_translation_en.name AS category_name_en,
        ...
        LEFT JOIN service_translations AS service_translation_zh
          ON service_translation_zh.shop_id = service.shop_id
         AND service_translation_zh.service_id = service.id
         AND service_translation_zh.locale = 'zh-CN'
        LEFT JOIN service_translations AS service_translation_en
          ON service_translation_en.shop_id = service.shop_id
         AND service_translation_en.service_id = service.id
         AND service_translation_en.locale = 'en'
        LEFT JOIN service_categories AS category
          ON category.shop_id = service.shop_id
         AND category.id = service.category_id
        LEFT JOIN service_category_translations AS category_translation_zh
          ON category_translation_zh.shop_id = category.shop_id
         AND category_translation_zh.category_id = category.id
         AND category_translation_zh.locale = 'zh-CN'
        LEFT JOIN service_category_translations AS category_translation_en
          ON category_translation_en.shop_id = category.shop_id
         AND category_translation_en.category_id = category.id
         AND category_translation_en.locale = 'en'
        ...
        ORDER BY category.sort_order ASC NULLS LAST,
                 category.id ASC NULLS LAST,
  ...
  const replaceStaffServices = async (request, response) => {
    ...
    try {
      ...
      if (typeof invalidateCatalog === 'function') invalidateCatalog(request.ownerAuth.shopId);
