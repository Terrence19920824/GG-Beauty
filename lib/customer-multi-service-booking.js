...
const planStaffAssignments = async ({ items, candidatesByService, validate, maxNodes = MAX_SEARCH_NODES }) => {
  ...
  const visit = async index => {
    ...
    const orderedCandidates = candidates.map((candidate, position) => ({ candidate, position,
      reservationPressure: items.slice(index + 1).reduce((score, later) => {
        if (later.staffSelectionType !== 'no_preference') return score;
        const laterCandidates = candidatesByService.get(later.serviceId) || [];
        return score + (laterCandidates.length === 1 && laterCandidates[0].staff_id === candidate.staff_id ? 1 : 0);
      }, 0)
    })).sort((left, right) => left.reservationPressure - right.reservationPressure || left.position - right.position);
