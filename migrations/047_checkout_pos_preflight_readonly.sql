...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.appointment_items GROUP BY shop_id,id HAVING count(*)>1) THEN RAISE EXCEPTION 'checkout preflight: appointment_items(shop_id,id) duplicates'; END IF;
